...
public class UpdateCategoryHandlerTests
{
    ...
    public async Task Given_ValidCommand_When_Handle_Then_CategoryIsUpdated()
    {
        ...
        var updated = await context.Set<DeveloperEvaluation.Domain.Entities.Category>().FindAsync(category.Id);
    ...
    public async Task Given_DuplicateNameForDifferentCategory_When_Handle_Then_ThrowsValidationException()
    {
        ...
        var existing1 = new DeveloperEvaluation.Domain.Entities.Category
        {
            Name = "Books",
            Description = "First"
        };
        var existing2 = new DeveloperEvaluation.Domain.Entities.Category
        {
            Name = "Movies",
            Description = "Second"
        };
        context.Set<DeveloperEvaluation.Domain.Entities.Category>().AddRange(existing1, existing2);
        await context.SaveChangesAsync();
