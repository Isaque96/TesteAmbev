...
public class ListProductsHandlerTests
{
    ...
    public async Task Given_ProductsExist_When_Handle_Then_PagedResultReturned()
    {
        ...
        for (var i = 0; i < 25; i++)
        {
            context.Set<DeveloperEvaluation.Domain.Entities.Product>().Add(new DeveloperEvaluation.Domain.Entities.Product
            {
                Id = Guid.NewGuid(),
                Title = $"Product {i:D2}",
                Price = 10 + i,
                Description = "Desc",
                Category = i % 2 == 0 ?
                    new DeveloperEvaluation.Domain.Entities.Category
                    {
                        Name = "electronics",
    ...
    public async Task Given_Filters_When_Handle_Then_FilteredProductsReturned()
    {
        ...
        context.Set<DeveloperEvaluation.Domain.Entities.Product>().AddRange(
            new DeveloperEvaluation.Domain.Entities.Product
            {
                ...
                Category = new DeveloperEvaluation.Domain.Entities.Category {Name = "electronics", Description = "Electronics"},
                Image = "img",
            ...
            new DeveloperEvaluation.Domain.Entities.Product
            {
                ...
                Category = new DeveloperEvaluation.Domain.Entities.Category {Name = "books", Description = "Books"},
                Image = "img",
    ...
    public async Task Given_ExactPageSizeProducts_When_Handle_Then_TotalPagesCorrect()
    {
        ...
        for (var i = 0; i < 20; i++)
        {
            context.Set<DeveloperEvaluation.Domain.Entities.Product>().Add(new DeveloperEvaluation.Domain.Entities.Product
            {
                ...
                Price = 10 + i,
                Description = "Desc",
                Category = new DeveloperEvaluation.Domain.Entities.Category{Name = "category", Description = "category"},
                Image = "img",
                Rating = new Rating { Rate = 4, Count = 10 }
            });
        }
